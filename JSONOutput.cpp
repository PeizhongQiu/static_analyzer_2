...
json::Object JSONOutputGenerator::convertHandlerAnalysis(const InterruptHandlerAnalysis& analysis) {
    ...
    if (high_confidence_writes > 3 || dev_id_accesses > 5) {
        priority = "HIGH";
    } else if (high_confidence_writes > 1 || dev_id_accesses > 2) {
        priority = "MEDIUM";
    }
...
json::Value JSONOutputGenerator::convertToJSON(const std::vector<InterruptHandlerAnalysis>& results) {
    ...
    for (const auto& analysis : results) {
        ...
        total_function_calls += analysis.function_calls.size();
    ...
    global_stats["total_function_calls"] = total_function_calls;
